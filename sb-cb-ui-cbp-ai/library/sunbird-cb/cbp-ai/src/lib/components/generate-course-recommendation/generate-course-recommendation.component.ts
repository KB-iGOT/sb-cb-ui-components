  ...
  getRecommendedCourseWithProgress(role_mapping_id: string): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      concat(
        ...
        interval(5000)     // poll every 5 sec
      )
        .pipe(
          switchMap(() =>
            this.sharedService.getRecommendedCourse(role_mapping_id)
          ),

          tap((response: any) => {

            if (!this.firstApiResponse && !this.existingRecommendationFound) {
              this.firstApiResponse = response;
              if (response?.is_existing) {
                this.existingRecommendationFound = true;
              }
              console.log(
          ...
          takeWhile((response: any) => {
            ...
            // stop polling if existing recommendations found
          ...
          next: (response: any) => {
            ...
            if (this.firstApiResponse?.is_existing && this.existingRecommendationFound) {
              ...
              const dialogRef = this.dialog.open(
                ...
                {
                  data: {
                    from: 'generateCourseRecommendations'                 // index for UI removal
                  },
                  width: '500px',
                  disableClose: true
                }
              ...
              dialogRef.afterClosed().subscribe(result => {
                ...
                if (result === 'saved') {
                  ...
                  this.sharedService
                    .deleteCourseRecommendations(role_mapping_id)
                    .subscribe({

                      next: () => {
                        this.existingRecommendationFound = false;
                        this.loading = false;
                        this.firstApiResponse = null;
                      ...
                      error: (error) => {
                        ...
                        this.snackBar.open(
                          error?.error?.detail ||
                          'Unable to delete recommendations',
                          'X',
                          {
                            duration: 3000,
                            panelClass: ['snackbar-error']
