    ...
    switch (apiDetailsKey) {
      ...
      case 'trainingPlanApi':
        ...
        data.forEach((item: any) => {
          if (item?.isApar === true || item?.planTypeV2 === 'AICBP' || item?.planTypeV2 === 'AI CBP-Non APAR') {
            return
          }
          const endDate = dayjs(item.endDate).format('YYYY-MM-DD')
      ...
      case 'draftCBPplanApi':
        ...
        const draftCBPFilteredData = data.filter((item: any) => item?.isApar !== true && (item?.planTypeV2 === 'AICBP' || item?.planTypeV2 === 'AI CBP-Non APAR'))
