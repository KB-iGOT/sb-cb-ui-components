...
export class HighlightsOfWeekComponent implements OnInit, OnChanges {
  ...
  loadContentData() {
    ...
    // If no items or empty list, add a default item
    if (!this.contentdata.length) {
      const defaultItem = {
        title: "What is iGOT Karmayogi?",
        description: "iGOT Karmayogi is an online learning platform that aims to transform government officials and governance in India.",
        videoUrl: "https://portal.qa.karmayogibharat.net/assets/public/content/guide-videos/Website_Video.mp4",
        cardSubType: "card-wide-lib"
      };
      this.contentdata.push(defaultItem);
    }
  }
